...
**Курс после аудита 05.09.2026:** исходные измерения и GIS получить невозможно.
Утверждённые названия диплома и специальной части сохраняются. Основной результат —
алгоритм прогноза по истории маркшейдерских наблюдений; реконструкция по публикациям
и сценарные проверки обеспечивают его разработку и оценку. Текущий план, объяснение gates и задачи
ускорения: [RESEARCH_DIRECTION_RU.md](docs/governance/RESEARCH_DIRECTION_RU.md).

Подготовлен [исправленный выпуск данных](data/reconstruction_research_v1/README.md):
каталог, происхождение и доступность полей, статусы наблюдений и пропуски.
Воспроизведение: `python scripts/repair_reconstruction_data.py --root .`.
Следующая задача — проверка исходных пространственных схем; C2 отложен.
[Передача локальному Codex](docs/prompts/LOCAL_CODEX_RECONSTRUCTION_HANDOFF_RU.md).

Ниже сохранено описание предыдущего эксперимента. Его метрики относятся к
родительской версии; прежний внешний holdout остаётся историческим протоколом.
Точность на реальном руднике из этих результатов не следует.
