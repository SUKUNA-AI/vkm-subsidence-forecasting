# VKM/SKRU-1 corpus re-sweep protocol — PHASE 1 (cloud, information/architecture only)
...
- `information_lifecycle` — how/when a class of information comes into existence. data: {"data_class": "exploration borehole log|lab test|mine survey of workings|levelling campaign|InSAR scene|GPR survey|backfill record|normative document|...", "event_time": "", "measurement_time": "", "processing_time": "", "publication_time": "", "availability_to_forecaster": "", "latency_notes": "", "who_produces": "", "where_stored": ""}
- `lifecycle_stage` — a stage of the physical mine/deposit lifecycle described in the source (formation, exploration, drilling, core, lab, logging, geophysics, geological model, mine planning, development, extraction, multi-seam sequence, backfill, long-term evolution, underground observation, surveying, surface response, closure/flooding...). data: {"stage": "", "description": "", "inputs": "", "outputs": "", "actors": "", "time_scale": ""}
- `open_question` — something the project needs but the source leaves unknown. data: {"question": "", "why_it_matters": ""}


## Scratch-file isolation (MANDATORY — collision incident 26.09)
Many readers run in parallel and share the session scratchpad. Generic helper names (b1.py, rec.py, batch.py)
collided and one reader executed another reader's script, which re-appended that reader's batch.
- Put ALL your helper scripts/temp files in a private subdirectory named after your chunk, e.g.
  `<scratchpad>/<SOURCE_ID>_<CHUNK_ID>/` or `/home/user/work/tmp/<SOURCE_ID>_<CHUNK_ID>/`, with chunk-prefixed names.
- Never execute a script you did not write in this run. Prefer inline heredocs over reusable files.
- After each append, verify that the last lines of YOUR records.jsonl are your batch (count lines before/after)
  and that no exact-duplicate lines were introduced; if a batch is missing, re-append it.

## Phase-1 scope rules
- This is an EXTRACTION / CATALOGUING pass. Do NOT perform numerical calculations of your own (no stress,
  creep integration, fitting, interpolation, recalculation of published examples). Copy the source's own
  numbers, equations and results exactly, with units and locators. Unit conversions are also deferred
  (record units as printed; you may note the obvious SI meaning in `notes`).
- Record formulas EXACTLY (notation, variables, units, equation number, original vs cited, known variants).
- Record information-lifecycle facts for every data class you meet: when the physical event happened,
  when it was measured, processed, published, and when it would have become available to an engineer or
  forecaster (kind `information_lifecycle`, see below).
...
## Outputs (write ONLY into your chunk directory) — INCREMENTAL SAVING IS MANDATORY
`/home/user/work/sweep_results/<SOURCE_ID>/<CHUNK_ID>/`

- Create the directory at the very start and write `progress.json` immediately:
  `{"source_id": "...", "chunk_id": "...", "pages_assigned": "...", "pages_done": [], "status": "IN_PROGRESS", "n_records": 0}`.
- Work in batches of AT MOST 5 pages. After each batch: APPEND that batch's records to `records.jsonl`
  (never rewrite earlier lines) and update `progress.json` (pages_done, n_records, last_batch note).
  A stop of the VM at any moment must lose at most one batch.
- At the end write `coverage.json` and set progress status to `DONE`.
- If a previous partial run of the same chunk exists in the directory (progress.json with pages_done),
  CONTINUE from the first page not in pages_done instead of starting over.
...
{"source_id": "", "chunk_id": "", "pages_assigned": "a-b",
 ...
 "open_questions": ["..."]}
```
3. Return (final answer) ONLY a compact JSON summary (the files are the real deliverable): {"source_id","chunk_id","n_records","n_new","n_boreholes","n_formulas","n_citations","n_visual_checks","key_findings":[<=8 short strings],"blockers":[...]}.
