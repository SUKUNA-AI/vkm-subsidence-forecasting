"""V2.1 exact-calendar erratum of the frozen v2 generator (V2-REP-001).
...
from .empirical_constraints_v2 import checked_inputs, inventory, NUMERIC
...
def exact_elapsed_days(current_date, previous_observed_date) -> int:
    """Calendar days are the authority; never recover them from fractional years."""
    return int((current_date - previous_observed_date) / np.timedelta64(1, "D"))


def empty_destination(root: Path, output: Path, declared: str) -> None:
    output = output.resolve()
    if declared != "data/scenario_simulation_v2_1":
        raise ValueError("V2.1 runner requires its own release destination")
    if output != root / declared and not output.is_relative_to(root / "work/scenario_v2_1_erratum"):
        raise ValueError("Output must be v2.1 release or work/scenario_v2_1_erratum")
    if output.exists() and any(output.iterdir()):
        raise FileExistsError("Immutable destination contains files")
...
def scenario_frames(
    ...
    for point_index in range(len(point_ids)):
        ...
        for position, campaign_index in enumerate(observed_indices):
            ...
            elapsed_days = exact_elapsed_days(dates[campaign_index], dates[previous])
            ...
            days_since_previous[campaign_index, point_index] = elapsed_days
...
def generate(root: Path, config_path: Path, output: Path) -> dict:
    ...
    validation["exact_calendar_elapsed_days"] = validate_calendar_elapsed(hist, features)
    sequences=sequence_windows(hist,features,config["sequence_max_length"])
    validation["window_chronology"] = validate_window_chronology(hist, features, sequences)
    ...
    code=[Path(__file__),root/"src/skru1/empirical_constraints_v2.py",root/"src/skru1/scenario_simulation.py",root/"scripts/generate_scenario_v2_1.py"]
    manifest=dict(schema_version=2,dataset_id=config["dataset_id"],source_release_id=config["source_release"]["dataset_id"],
        ...
        erratum=config["erratum"],
        command="python scripts/generate_scenario_v2_1.py --root .")
    _write_json(output/"manifest.json",manifest)
    return validation


def validate_calendar_elapsed(history: pd.DataFrame, features: pd.DataFrame) -> dict:
    ordered = history.sort_values(["point_id", "current_date"]).copy()
    ordered["exact_days"] = pd.to_datetime(ordered.current_date).groupby(ordered.point_id).diff().dt.days
    joined = features.merge(ordered[["point_id", "current_date", "exact_days"]],
                            on=["point_id", "current_date"], validate="many_to_one", how="left")
    bad = joined.exact_days.isna() | joined.exact_days.ne(joined.days_since_previous_observation)
    if bad.any():
        raise ValueError(f"V2-REP-001 exact-calendar invariant failed: {int(bad.sum())}")
    return dict(origins_checked=len(joined), mismatch_count=0, authority="integer_calendar_timedelta")


def validate_window_chronology(history, features, windows) -> dict:
    histories = {}
    for point, group in history.groupby("point_id", sort=False):
        group = group.sort_values("current_date")
        histories[point] = (group.history_id.tolist(), dict(zip(group.current_date, range(len(group)))))
    if not windows.sample_id.equals(features.sample_id):
        raise ValueError("Window/origin identity mismatch")
    for row in windows.itertuples(index=False):
        ids, dates = histories[row.point_id]
        ...
        if json.loads(row.history_ids_json) != expected or row.sequence_length != len(expected) or row.left_padding != 16-len(expected):
            raise ValueError("Window chronology/cutoff mismatch")
    return dict(origins_checked=len(windows), mismatch_count=0)
