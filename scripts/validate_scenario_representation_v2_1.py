...
from skru1.scenario_boundary_v2_1 import DATA_DIRECTORY, DATA_SHA, CONSTRAINTS_SHA, file_sha256
from skru1.scenario_scorer_v2_1 import load_campaign_plan_for_qa, load_evaluator_truth
...
def validate_inputs(root: Path) -> dict:
    ...
    gap_mismatches = int(joined.days_since_previous_observation.ne(joined.gap_days_check).sum())
    check("origin_gap", gap_mismatches == 0)
    ...
    return {"status": "PASS" if passed else "FAIL", "checks": checks,
            ...
            "elapsed_day_mismatches": gap_mismatches,
