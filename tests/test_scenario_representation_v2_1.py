...
from skru1.scenario_adapter_v2_1 import (
    FEATURES, METADATA, COUNTS, TARGET, load_model_data, load_calibration_targets,
    estimator_matrix, CausalHistory,
)
from skru1.scenario_boundary_v2_1 import (
    DATA_SHA, CONSTRAINTS_SHA, DATA_DIRECTORY, MODEL_FILES, file_sha256, model_worker_scope,
)
from skru1.scenario_scorer_v2_1 import load_evaluator_truth
from skru1.scenario_sequences_v2_1 import SequenceTensorizer, CHANNELS
from skru1.scenario_splits_v2_1 import (
...
from skru1.scenario_worker_v2_1 import run_origin_worker, origin_inputs
...
@pytest.mark.parametrize("relative", [
    "data/scenario_simulation_v2_1/evaluator/next_planned_truth.csv.gz",
    "data/scenario_simulation_v2_1/evaluator/campaign_truth.csv.gz",
    "data/scenario_simulation_v2_1/scenario_catalog.csv",
    "data/scenario_simulation_v2_1/targets/calibration_observed.csv.gz",
    ...
    "data/scenario_simulation_v2/model_features.csv.gz",
])
...
def test_worker_blocks_unguarded_child_process():
    import subprocess
    with model_worker_scope(ROOT), pytest.raises(PermissionError, match="launch a process"):
        subprocess.run([sys.executable, "-c", "raise RuntimeError('must never start')"], check=True)


def test_truth_store_is_scorer_only_and_join_is_exact(bundle):
    store = load_evaluator_truth(ROOT)
    with pytest.raises(TypeError):
        run_origin_worker(ROOT, store, bundle.frames.sample_id.iloc[0], lambda inputs: None)
    ids = bundle.evaluation.sample_ids[:3][::-1]
    dummy = pd.DataFrame({"sample_id": ids, "predicted_rate_mm_y": np.zeros(3)})
    joined = store.join_predictions(dummy)
    assert tuple(joined.sample_id) == ids
    np.testing.assert_array_equal(joined.latent_rate_mm_y,
                                  store._truth.set_index("sample_id").loc[list(ids), "latent_rate_mm_y"])
    with pytest.raises(ValueError):
        store.join_predictions(pd.concat([dummy, dummy.iloc[:1]]))
    with pytest.raises(ValueError):
        store.join_predictions(dummy.assign(sample_id="UNKNOWN"))
...
def test_all_runtime_windows_against_independent_token_gather(bundle, tensorizer):
    ...
    for batch in tensorizer.batches(bundle.frames.sample_id, batch_size=4096):
        ...
        for channel_index, name in enumerate(CHANNELS):
            np.testing.assert_array_equal(batch.values[:, -1, channel_index], origins[name])
...
def test_preprocessing_exact_fit_only(bundle, groups, folds, tensorizer, kind):
    ...
    assert "category_mappings" in state and "parameters" in state
    with pytest.raises(ValueError):
    ...
    # Poison tabular validation/calibration/evaluation too, not only history.
    poisoned_train = bundle.train.frame.copy()
    outside_fit = ~poisoned_train.sample_id.isin(fold.fit_ids)
    for name in FEATURES:
        poisoned_train.loc[outside_fit, name] = "UNSEEN_POISON" if name.endswith("campaign_type") else 1e12
    poisoned = replace(poisoned, train=replace(bundle.train, frame=poisoned_train),
                       calibration=replace(bundle.calibration, frame=bundle.calibration.frame.assign(last_settlement_mm=1e12)),
                       evaluation=replace(bundle.evaluation, frame=bundle.evaluation.frame.assign(last_settlement_mm=1e12)))
